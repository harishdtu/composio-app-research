...
Run:
    python analyze.py > ../data/analytics.json
"""
...
# ---------------------------------------------------------------------
# Primary authentication distribution
# ---------------------------------------------------------------------
...
for a in apps:
    ...
    bl = blocker.lower().replace("-", " ")

    if (
        "developer token" in bl
        or "app review" in bl
        or "partner program" in bl
        or "approval" in bl
        or "review" in bl
    ):
        blocker_buckets["Platform review / approval gate"] += 1

    elif (
        "enterprise" in bl
        or "existing customer" in bl
        or ("active" in bl and "account" in bl)
    ):
        blocker_buckets["Existing paid/enterprise account required"] += 1

    elif (
        "paid plan" in bl
        or "paid subscription" in bl
        or "free tier" in bl
        or "free api" in bl
        or "starter plan" in bl
    ):
        blocker_buckets["Paid-plan-only API"] += 1

    elif (
        "no public" in bl
        or "not publicly" in bl
        or "could not locate" in bl
        or "no discoverable" in bl
        or "standalone public" in bl
    ):
        blocker_buckets["No public developer docs / fully sales-gated"] += 1

    elif (
        "local tool" in bl
        or "hosted endpoint" in bl
        or "cli" in bl
        or "self hosted" in bl
        or "self-hosted" in bl
    ):
        blocker_buckets["Not a hosted API (CLI/local-only tool)"] += 1
...
mcp_ready_apps = [
    a["app"]
    for a in apps
    if a["mcp"] in ("official", "community")
]
...
output = {
    ...
    "access_distribution": {
        ...
        for k, v in access_dist.most_common()
    },
    "verdict_distribution": {
        ...
        for k, v in verdict_dist.most_common()
    },
    "mcp_distribution": {
        k: {
            "count": v,
        ...
        for k, v in mcp_dist.most_common()
    },
